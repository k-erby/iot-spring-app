...
public class Hub extends Device implements Mediator {
    ...
    for (Client c : aClients.values()) {
      for (User u : c.getUsers()) {
        if (u.getDevices().contains(pDevice)) {
          ...
          c.notify(u, new JSONMessaging(pDevice, pMessage).invoke());
        }
      }
    }
  }
  ...
  public void alert(User pUser, Device pDevice, String pMessage) {
    ...
    INSTANCE.notify(pUser, new JSONMessaging(pDevice, pMessage).invoke());
  }

  // Creates a notification and logs it. Can be called by any class with a Hub object for logging
  // purposes.
      ...
      for (Client c : aClients.values()) {
        for (User u : c.getUsers()) {
          if (u.getDevices().contains(d)) {
            ...
            c.notify(u, new JSONMessaging(d, message).invoke());
          }
        }
      }
    }
  ...
  public void alert(String message) {
    ...
    for (Client c : aClients.values()) {
      for (User u : c.getUsers()) {
        c.notify(u, new JSONMessaging(message).invoke());
      }
    }
  }
  ...
  /**
   * If a device detects activity, it asks the Hub to check if any actions should be taken with the
   * other devices e.g. if Camera detects activity and Lightbulbs are off then turn lights on or if
   * Camera doesn't detect activity and Lightbulbs are on then turn lights off or User has settings
   * that when Lightbulb turn on in a room and it is Winter, raise Thermostat or etc.
   *
