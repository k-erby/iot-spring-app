...
public class User {
  ...
  @Transient private boolean isSignedIn = false;

  @Transient private ArrayList<Device> aDevices = new ArrayList<Device>();

  @Transient private Queue<JSONObject> notifications;

  // username, password
  @Transient private HashMap<String, String> aUsers = new HashMap<String, String>();
  ...
  public void manageDevices(User u, Device d, boolean remove) {
    ...
    if (this.getIsAdmin()) {
      if (remove) {
        u.unregisterDevice(
            d); // TODO: make this happen in hub too. make static? or move this method in Hub
      }
    } else {
      u.registerDevice(d);
    }
  ...
  public String toString() {
    return String.format("User[username=%d, ID='%s']", username, getIdentifier().toString());
  }
}
